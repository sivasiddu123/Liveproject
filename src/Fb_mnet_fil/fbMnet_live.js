...
for(var i=0;i<facebook.length;i++){
    for(var j=0;j<mnet.length;j++){
        if(facebook[i].adset_id===mnet[j].adset_id && facebook[i].BSTHour===mnet[j].BSTHour && facebook[i].BSTDate===mnet[j].BSTDate){ 
            // var a={...facebook[i],...mnet[j]};
            shop.push({...facebook[i],...mnet[j]})
        }
    }
}
