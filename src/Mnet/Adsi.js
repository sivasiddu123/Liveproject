// customer_guid=6537AC5A-63D3-4A13-AA3F-3B9929D37E5A&customer_key=8CUF353U5
...
async function sum(){
    var pages=[]
    const response =await axios.get(`https://pubconsole.media.net/api/reports/v1/hourly-channel-wise?customer_guid=6537AC5A-63D3-4A13-AA3F-3B9929D37E5A&customer_key=8CUF353U5&from_date=${yesterday}&to_date=${today}&page_size=5000&page_number=1`)
    ...
    for(var i=2;i<=len;i++){
        const response2 =await axios.get(`https://pubconsole.media.net/api/reports/v1/hourly-channel-wise?customer_guid=6537AC5A-63D3-4A13-AA3F-3B9929D37E5A&customer_key=8CUF353U5&from_date=${yesterday}&to_date=${today}&page_size=5000&page_number=${i}`)
...
async function db(){
  const client = await mClient()
          //    console.log("slope", slope)
             ...
             await client.db("LocalTest").collection("Mnet_ABD").deleteMany({name:"Adsi",delet_date:today1});
            // //  moment_mnet
