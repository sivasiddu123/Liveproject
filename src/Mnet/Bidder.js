...
async function sum(){
    ...
    // var yesterday = moment().subtract(1, "days").format("MM/DD/YYYY")
    // var today = moment().format("MM/DD/YYYY")
    const response =await axios.get(`https://pubconsole.media.net/api/reports/v1/hourly-channel-wise?customer_guid=CF6FF673-0F7A-44BF-8F26-DC2CE5878740&customer_key=8CU2R1JY7&from_date=${yesterday}&to_date=${today}&page_size=5000&page_number=1`)
    ...
    for(var i=2;i<=len;i++){
        const response2 =await axios.get(`https://pubconsole.media.net/api/reports/v1/hourly-channel-wise?customer_guid=CF6FF673-0F7A-44BF-8F26-DC2CE5878740&customer_key=8CU2R1JY7&from_date=${yesterday}&to_date=${today}&page_size=5000&page_number=${i}`)
...
async function db(){
  const client = await mClient()
          //    console.log("slope", slope)
             ...
             await client.db("LocalTest").collection("Mnet_ABD").deleteMany({name:"Bidder",delet_date:today1});
            // //  moment_mnet
